...
class IOnloginSettings(Interface):
    ...
    first_login_redirect_enabled = schema.Bool(
        ...
        description=_(u'First login redirect is of higher priority than '
                      'redirect on every login. If disabled, then every login '
                      'redirect will be executed even for first time login.'),
        required=False,
        default=True)
    ...
    login_redirect_enabled = schema.Bool(
        ...
        description=_(u'Login redirect is happening on every user login and '
                      'does not happen on user first login in case First '
                      'Login Redirect is enabled on this form above.'),
        required=False,
        default=True)
