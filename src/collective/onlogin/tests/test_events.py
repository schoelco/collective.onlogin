...
from collective.onlogin.events import userInitialLogin
from collective.onlogin.events import userLogin
...
class EventsTests(unittest.TestCase):
    ...
    def test_userLogin_disable_redirect(self):
        ...
        # check if our function doesn't redirect in our case the header
        # location doesn't change when the onlogin redirect checkbox is set
        # to False
    ...
    def test_userLogin_enable_initial_login(self):
        ...
        # check when function doesn't redirect because of providing the
        alsoProvides(portal.REQUEST, IUserInitialLoginInEvent)
        portal.REQUEST.RESPONSE.setHeader('Location',
                                          'affecting_initial_login')
    ...
    def test_userLogin_enable_ignoring_came_from(self):
        ...
        # set first onlogin checkbox to disable and no longer provide the
        # initial login
        registry['collective.onlogin.interfaces.IOnloginSettings.'
    ...
    def test_userLogin_none_expression(self):
        ...
        # checking the header when expression of where to redirect is None
        registry['collective.onlogin.interfaces.IOnloginSettings.'
    ...
    def test_userLogin_with_expression(self):
        ...
        # finally setting suitable expression
        registry['collective.onlogin.interfaces.IOnloginSettings.'
                 'login_redirect_expr'] = u'string:some_url'

        # setting some header which must be changed on getting expression
        portal.REQUEST.RESPONSE.setHeader('Location', 'not_using')
        userLogin(portal, None)
        self.assertEqual('some_url',
                         portal.REQUEST.RESPONSE.getHeader('Location'))

        # and checking case when came_from attribute is not None the same way
        registry['collective.onlogin.interfaces.IOnloginSettings.'
        ...
        # set the ignoring_came_from because the default value is False
        registry['collective.onlogin.interfaces.IOnloginSettings.'
    ...
    def test_userInitialLogin_none_expression(self):
        ...
        # checking the header when expression of where to redirect is
        # None
        registry['collective.onlogin.interfaces.IOnloginSettings.'
